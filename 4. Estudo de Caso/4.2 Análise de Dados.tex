...
As matrizes de Pearson e Spearman (Figuras~\ref{fig:corr} e \ref{fig:corr1}) indicaram que a transformação cíclica da hora (\textit{hora\_sazonal}) é o preditor mais relevante, apresentando a maior correlação direta com a irradiância, seguida da temperatura do ar (correlação positiva moderada) e da umidade relativa (correlação negativa moderada a forte). Também a sazonalidade anual transformada (\textit{mes\_sazonal}) mostrou associação superior à do mês bruto, evidenciando que representações cíclicas favorecem a captura dos padrões intradiários e sazonais. Por outro lado, variáveis como \textit{dia}, \textit{minuto}, \textit{ano} e pressão atmosférica exibiram correlações muito baixas, sugerindo menor potencial preditivo isolado.

As relações entre preditores revelaram redundâncias esperadas, especialmente entre temperatura, umidade relativa e a própria sazonalidade anual, reforçando a necessidade de parcimônia e regularização em modelos sensíveis à multicolinearidade. Em modelos não lineares como XGBoost e LSTM, essas redundâncias são menos problemáticas, mas a adoção das codificações cíclicas permanece recomendada por simplificar padrões temporais. Em síntese, os resultados orientam a priorização de \textit{hora\_sazonal}, temperatura, umidade relativa e \textit{mes\_sazonal} como preditores centrais, enquanto as demais variáveis devem ser incluídas apenas quando agregarem informação complementar relevante.
...
\begin{figure}[!h]
    ...
    \includegraphics[width=0.85\textwidth]{Figuras/Pearson.png}
    \par\small{Fonte: Autor (2025)}
    \label{fig:corr}
\end{figure}
...
\begin{figure}[!h]
    ...
    \includegraphics[width=0.85\textwidth]{Figuras/Spearman.png}
    \par\small{Fonte: Autor (2025)}
    \label{fig:corr1}
\end{figure}
\FloatBarrier
