...
Os modelos avaliados apresentam naturezas distintas e complementares: o ARIMA representa a abordagem estatística clássica, baseada em pressupostos de linearidade e estacionaridade, sendo adequado para séries com padrões temporais estáveis e dependências lineares explícitas; a LSTM, por sua vez, pertence ao aprendizado profundo e é capaz de capturar relações não lineares e dependências de curto e longo prazo sem necessidade de estacionaridade, destacando-se em cenários de alta variabilidade como a previsão de irradiância influenciada por nebulosidade; já o XGBoost, modelo de \emph{gradient boosting} baseado em árvores, oferece um equilíbrio entre desempenho e interpretabilidade, aprendendo relações não lineares a partir de variáveis de atraso e janelas deslizantes, sendo menos dependente de grandes volumes de dados do que redes neurais profundas. Em conjunto, esses modelos evidenciam diferentes capacidades de representação temporal: ARIMA para padrões lineares simples, LSTM para dinâmicas complexas e XGBoost como alternativa robusta e flexível quando acompanhado de engenharia de variáveis adequada.
