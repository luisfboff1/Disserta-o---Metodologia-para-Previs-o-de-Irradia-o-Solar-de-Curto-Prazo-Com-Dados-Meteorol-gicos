...
A série multivariada será transformada em amostras supervisionadas por meio da definição de \textbf{janela de entrada} (comprimento $L$), \textbf{janela de saída} (horizonte $H$) e \textbf{passo} (\textit{stride} $s$). Cada amostra $n$ será construída conforme a Equação (\ref{eq:janela_superv}):
\begin{equation}
...
\noindent
Em que $\mathbf{x}_t \in \mathbb{R}^{p}$ conterá variáveis meteorológicas, componentes sazonais (seno/cosseno) e variáveis derivadas quando aplicável; $\mathbf{X}^{(n)} \in \mathbb{R}^{L \times p}$ agregará $L$ instantes passados; e $\mathbf{y}^{(n)} \in \mathbb{R}^{H}$ representará os $H$ passos futuros da irradiância. O índice $t$ avançará com passo $s$ (tipicamente $s=1$), permitindo gerar amostras sobrepostas (maior densidade de treinamento) ou não sobrepostas (menor autocorrelação entre amostras), conforme o objetivo experimental. O valor de $L$ deverá capturar memória temporal suficiente, enquanto $H$ refletirá o horizonte operacional de interesse.

\begin{itemize}
    \item \textbf{LSTM}: lotes de tamanho $B$ produzirão tensores $\mathbf{X}_{\mathrm{batch}} \in \mathbb{R}^{B \times L \times p}$, e a previsão multi-etapas será representada por $\hat{\mathbf{y}} \in \mathbb{R}^{B \times H}$.
    \item \textbf{XGBoost}: a janela será \textit{vetorizada} em $\mathbf{u}^{(n)} \in \mathbb{R}^{L \cdot p}$ ou expandida em estatísticas agregadas (médias, mínimos, máximos, últimas $k$ observações). O treinamento poderá ocorrer em formato \textit{multi-output} (um único modelo para $H$ saídas) ou \textit{direct per horizon} (um modelo independente para cada $h\in\{1,\dots,H\}$).
\end{itemize}
...
Será adotado o \texttt{MLflow} para rastrear parâmetros, métricas, artefatos e versões de modelos. A motivação será garantir reprodutibilidade, auditoria e comparabilidade sistemática.
